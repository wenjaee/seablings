        ...
        <div className="mt-4 space-y-4">
          {shouldShowAggregateCriteria ? (
            <div className="rounded-[22px] bg-[var(--zx-surface)] px-4 py-3">
              <p className="text-[12px] text-[var(--zx-muted)]">Session criteria</p>
              {aggregateAvailability ? <p className="mt-1.5 break-words text-[14px] font-medium text-[var(--zx-ink)]">Availability: {aggregateAvailability}</p> : null}
              {aggregateBudget ? <p className="mt-1.5 break-words text-[14px] text-[var(--zx-ink)]">Budget: {aggregateBudget}</p> : null}
              {aggregateVetoes ? <p className="mt-1.5 break-words text-[14px] text-[var(--zx-ink)]">Vetoes: {aggregateVetoes}</p> : null}
            </div>
          ) : null}
          ...
          <div className="rounded-[22px] bg-[var(--zx-surface)] px-4 py-3">
            ...
            <p className="mt-1 break-words text-[20px] font-semibold leading-tight text-[var(--zx-ink)]">
            ...
            {session.finalPlan?.proposedTime ?? session.proposedTime ? (
              <p className="mt-2 inline-flex min-w-0 flex-wrap items-center gap-2 break-words text-[13px] text-[var(--zx-muted)]">
                <Clock3 size={14} aria-hidden />
                {session.finalPlan?.proposedTime ?? session.proposedTime}
              </p>
            ) : null}
          ...
          <div className="space-y-3">
            {winningItems.map((item) => (
              <article key={item.id} className="rounded-[22px] border border-black/6 bg-white px-4 py-3">
                <div className="flex min-w-0 items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="break-words text-[16px] font-semibold text-[var(--zx-ink)]">{item.title}</p>
                    <p className="mt-1 break-words text-[13px] text-[var(--zx-muted)]">{getAreaLabel(item)}</p>
                  </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="rounded-[22px] bg-[var(--zx-surface)] px-4 py-3">
              ...
              <p className="mt-2 break-words text-[14px] text-[var(--zx-ink)]">
                {winningItems.map((item) => getAreaLabel(item)).join(" + ") || "TBD"}
              </p>
            </div>
            <div className="rounded-[22px] bg-[var(--zx-surface)] px-4 py-3">
              <p className="inline-flex items-center gap-2 text-[12px] font-semibold uppercase tracking-[0.14em] text-[var(--zx-faint)]">
              ...
              <p className="mt-2 break-words text-[14px] text-[var(--zx-ink)]">{getBudgetSummary(winningItems)}</p>
            </div>
          </div>
              ...
              <div className="flex flex-wrap gap-2">
                {voteEntries.map((entry) => (
                  <span
                    ...
                    className="max-w-full break-words rounded-full bg-white px-3 py-1.5 text-[12px] font-medium text-[var(--zx-ink)] shadow-[0_6px_16px_rgba(15,23,42,0.05)]"
                  >
